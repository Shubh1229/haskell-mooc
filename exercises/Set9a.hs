...
countValid [] = 0
countValid (x:xs) = if checkValidity 1 Nothing Nothing x then 1 + countValid xs else 0 + countValid xs
  where
    checkValidity :: Int -> Maybe Char -> Maybe Char -> [Char] -> Bool
    checkValidity _ _ _ [] = False
    checkValidity n Nothing Nothing (x:xs)  | n < 3 = checkValidity (n + 1) Nothing Nothing xs
                                  | n == 3 = checkValidity (n + 1) (Just x) Nothing xs
    checkValidity 4 (Just thrd) Nothing (x:xs) = checkValidity 5 (Just thrd) (Just x) xs
    checkValidity 5 (Just thrd) (Just frth) (x:xs) = if thrd == x then True else checkValidity 6 (Just thrd) (Just frth) xs
    checkValidity 6 _ (Just frth) (x:xs) = frth == x
...
sumSuccess xs = if noRights xs then findSum xs 0 else Left "no data"
  where
    noRights [] = False
    noRights (x:xs) = case x of
      Right _ -> True
      _ -> noRights xs
    findSum [] n = Right n
    findSum (x:xs) n = case x of
      Right y -> findSum xs (n + y)
      _ -> findSum xs n
...
open code (Locked password) = if code == password then Unlocked password else Locked password
...
instance Eq Text where
  (==) (Text x) (Text y) = removeSpacesAndNewLines x == removeSpacesAndNewLines y
    where
      removeSpacesAndNewLines [] = []
      removeSpacesAndNewLines (x:xs) = if isSpace x || '\n' == x then removeSpacesAndNewLines xs else x : removeSpacesAndNewLines xs
...
compose ((a,b):as) cds = case lookup b cds of
  Just x -> (a,x) : compose as cds
  Nothing -> compose as cds
compose [] _ = []
...
permute [] _ = []
permute xs as = map snd (sortOn fst (zip xs as))
