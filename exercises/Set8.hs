...
rectangle x0 y0 w h = Shape r
  where
    r (Coord x y) = x >= x0 && y >= y0 && x <= x0 + w - 1 && y <= y0 + h - 1
------------------------------------------------------------------------------
...
union (Shape x) (Shape y) = Shape (\coord -> x coord || y coord)

cut :: Shape -> Shape -> Shape
cut (Shape x) (Shape y) = Shape (\coord -> x coord && not (y coord))
------------------------------------------------------------------------------
...
paintSolid color (Shape s) (Picture base) = Picture g
  where
    g coord | s coord   = color
...
paint (Picture pat) (Shape s) (Picture base) = Picture g
  where
    g coord | s coord   = pat coord
            | otherwise = base coord
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where
      g (Coord x y) = avg5 (f (Coord x y))
                           (f (Coord (x-1) y))
                           (f (Coord (x+1) y))
                           (f (Coord x (y-1)))
                           (f (Coord x (y+1)))
      avg5 (Color r1 g1 b1) (Color r2 g2 b2) (Color r3 g3 b3) (Color r4 g4 b4) (Color r5 g5 b5) =
