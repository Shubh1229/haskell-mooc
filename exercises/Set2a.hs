...
palindromify s
    ...
    | otherwise = palindromify (chop s)
        where
            chop str = drop 1 (take (length str - 1) str)
...
safeIndex xs i
    ...
    | i == 0 = Just (head xs)
    | otherwise = safeIndex (drop 1 xs) (i - 1)
...
addEithers (Left str) (Left _) = Left str
addEithers (Right a) (Left str) = Left str
addEithers (Left str) (Right _) = Left str
