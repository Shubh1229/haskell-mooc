...
firstHalf :: [a] -> [a]
firstHalf str = take (half str) str
    where
        half str = if length str `mod` 2 == 0 then length str `div` 2 else length str `div` 2 + 1
...
joinToLength n xs = filter hasRightLegnth allJoins
    where
        allJoins :: [String]
        allJoins = concat (map joinsWith xs)
        joinsWith :: String -> [String]
        joinsWith x = map (\y -> x ++ y) xs
        hasRightLegnth :: String -> Bool
        hasRightLegnth str = if length str == n then True else False
...
multiCompose [] vr = vr
multiCompose (fs:rest) vr = fs (multiCompose rest vr)
