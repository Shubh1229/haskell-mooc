...
leftpad str n
    | n <= length str = str
    | otherwise = spaces (n - length str) ++ str
        where
            spaces x
                | x == 0 = ""
...
countdown x = "Ready! " ++ count x ++ "Liftoff!"
    where
        count y
            | y <= 0 = ""
            | otherwise = show y ++ "... " ++ count (y - 1)
...
smallestDivisor n
    | n `mod` 2 == 0 = 2
    | otherwise = findSmallestDiv n 3
        where
            findSmallestDiv x y
                | x `mod` y == 0 = y
                | otherwise = findSmallestDiv x (y + 1)
