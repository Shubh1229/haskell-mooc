...
treeMax t = case t of
    Empty -> 0
    Node a left right -> max a (max (treeMax left) (treeMax right))
...
allValues condition tree = case tree of
  Empty -> True
  Node a left right
    | condition a -> allValues condition left && allValues condition right
    | otherwise -> False
...
isOrdered t = case t of
  Empty -> True
  Node a left right -> allValues (< a) left && allValues (> a) right && isOrdered left && isOrdered right
...
walk [] tree = case tree of
  Empty -> Nothing
  Node a _ _ -> Just a
walk (step:rest) tree = case tree of
  Empty -> Nothing
  Node a left right
    | step == StepL -> walk rest left
    | otherwise -> walk rest right
...
set [] val tree = case tree of
  Empty -> tree
  Node a left right -> Node val left right
set (path:rest) val tree = case tree of
  Empty -> tree
  Node a left right
    | path == StepL -> Node a (set rest val left) right
    | otherwise -> Node a left (set rest val right)
