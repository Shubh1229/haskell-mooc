...
sums i
    ...
    | i > 1 = helpSums i [] 1
        where
            helpSums i lst index = if index <= i then findSums index : helpSums i lst (index + 1) else []
            findSums i = if i > 0 then i + findSums (i - 1) else 0
...
sorted [] = True
sorted [x] = True
sorted [a,b] = if a <= b then True else False
