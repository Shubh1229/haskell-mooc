...
data Person = PersonUndefined | Person Int String
  deriving Show
...
data Position = PositionUndefined | Position Int Int
...
rgb col = case col of
  ...
  Green -> [0,1,0]
  Blue -> [0,0,1]
  Mix color1 color2 -> zipWith findAverage (rgb color1) (rgb color2)
    where
      findAverage x y = (x + y) / 2
  Invert color -> map (\x -> 1 - x) (rgb color)
...
prettyPrint b = case b of
  End -> ""
  O next -> prettyPrint next ++ "0"
  I next -> prettyPrint next ++ "1"

fromBin :: Bin -> Int
fromBin b = case b of
  End -> 0
  O next -> 2 * fromBin next
  I next -> 1 + 2 * fromBin next
